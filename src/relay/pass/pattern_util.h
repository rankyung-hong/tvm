...
inline Expr Sum(Expr data, Array<Integer> axis, bool keepdims, bool exclude) {
  auto attrs = make_node<ReduceAttrs>();
  attrs->axis = std::move(axis);
  attrs->keepdims = keepdims;
  attrs->exclude = exclude;
...
inline Expr AvgPool2D(Expr data,
...
inline Expr Pad(Expr data, Array<Array<IndexExpr>> pad_width, double pad_value) {
