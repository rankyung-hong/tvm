...
Expr Conv2DFallBack(const Expr& data, const Expr& weight, const Expr& zp_data,
  ...
  auto shifted_data = Subtract(Cast(data, Int(32)), zp_data);
  auto shifted_kernel = Subtract(Cast(weight, Int(32)), zp_kernel);
 ...
 *       * Combine the terms.
 */
Expr QnnConv2DLegalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  CHECK(param->kernel_layout == "OIHW" || param->kernel_layout == "HWIO" || param->kernel_layout ==
          "HWOI") << "qnn.conv2d supports only OIHW/HWIO/HWOI kernel data layout.";
